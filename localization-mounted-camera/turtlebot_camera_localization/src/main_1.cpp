...
void imageCallback(const sensor_msgs::ImageConstPtr& original_image)
{
    ...
    if (CParam.isValid()) {
		...
		camPose.pose.pose.position.y = (420-marker_centroid.y)*SCALING_FACTOR_Y-500; //SWITCH TO -500 FOR 2CAM
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle nh;
        ...
        image_transport::Subscriber sub = it.subscribe("/camera_1/image_raw", 1, imageCallback);
...
odom_pub_map[_PICASSO] = nh.advertise<nav_msgs::Odometry>("picasso/odom", 2);
	camPose_pub_map[_PICASSO] = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("picasso/amcl_pose", 1, true);
	particleCloudPub_map[_PICASSO] = nh.advertise<geometry_msgs::PoseArray>("picasso/particlecloud",1,true);  

        pub = it.advertise("/camera_1/image_processed", 1);
	...
	CParam.readFromXMLFile("/home/aaron/catkin_ws/src/localization-mounted-camera/turtlebot_camera_localization/camera_old.yml");
