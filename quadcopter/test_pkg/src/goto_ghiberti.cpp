...
pos_sub_ = nh_.subscribe<turtlebot_deployment::PoseWithName>("ghiberti/afterKalman", 1, poseCallback);
cen_sub_ = nh_.subscribe<geometry_msgs::PoseStamped>("ghiberti/move_base_simple/goal",1, updateCentroid);
geometry_msgs::Twist finalVel;

u_pub_ = nh_.advertise<geometry_msgs::Twist>("/ghiberti/cmd_vel_mux/input/navi", 1, true);
